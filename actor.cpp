...
Actor::Actor(std::string name, Glyph glyph, Cell* startingCell, bool isPlayer) :
...
void Actor::Act(int x, int y, Map& map)
{
...
void Actor::Defend(float accuracy, size_t damage)
{
	MTEngine random;
...
void Actor::Draw(const GlyphType& glyphType, const raylib::Vector2 screenPosition) const
{
	glyphType.DrawGlyph(m_Glyph, raylib::Rectangle(m_Position.x * glyphWidth + screenPosition.x, m_Position.y * glyphHeight + screenPosition.y, glyphWidth, glyphHeight));
}
